...
def _dataset_to_arrays(dataset_obj):
    """
    Convert a torch Dataset into full numpy arrays.

    Returns
    -------
    X : np.ndarray
        Shape (n_samples, win_size, n_features)
    y : np.ndarray
        Shape (n_samples, win_size)
    """
    X_list = []
    y_list = []
    ...
    for i in range(len(dataset_obj)):
        ...
        if isinstance(item, (list, tuple)):
            if len(item) >= 2:
                x_i, y_i = item[0], item[1]
            else:
                raise ValueError(f"Dataset item at index {i} has invalid length: {len(item)}")
        else:
            raise ValueError(f"Dataset item at index {i} is not tuple/list: {type(item)}")

        X_list.append(np.asarray(x_i, dtype=np.float32))
        y_list.append(np.asarray(y_i, dtype=np.float32))
...
def load_dataset_for_cv(
    ...
    """
    Load dataset through the existing data loader pipeline and export
    already-windowed numpy arrays for CV.
    ...
    win_size : int
        Window size.
    input_c : Optional[int]
        Expected feature dimension.
    batch_size : int
        Required by get_loader_segment.
    source : str
        Which split to export:
          ...
          - "train_val"
    step : Optional[int]
        Sliding step. If None, defaults to win_size.
    ...
    y : np.ndarray
        Shape (n_samples, win_size)
    """
    if source not in {"train", "val", "test", "train_val"}:
        raise ValueError(
            f"Unsupported source={source!r}. "
            f"Use one of ['train', 'val', 'test', 'train_val']."
        )

    if step is None:
        step = win_size

    def _build_split_loader(mode):
    ...
    if source == "train":
        loader = _build_split_loader("train")
        X, y = _dataset_to_arrays(loader.dataset)

    elif source == "val":
        loader = _build_split_loader("val")
        X, y = _dataset_to_arrays(loader.dataset)

    elif source == "test":
        loader = _build_split_loader("test")
        X, y = _dataset_to_arrays(loader.dataset)
        ...
        X_train, y_train = _dataset_to_arrays(train_loader.dataset)
        X_val, y_val = _dataset_to_arrays(val_loader.dataset)

        X = np.concatenate([X_train, X_val], axis=0)
        y = np.concatenate([y_train, y_val], axis=0)

    if X.ndim != 3:
        raise ValueError(f"Loaded X must be 3D, got shape {X.shape}")
    if y.ndim != 2:
        raise ValueError(f"Loaded y must be 2D, got shape {y.shape}")

    if X.shape[1] != win_size:
        raise ValueError(
            f"Loaded window size mismatch: X.shape[1]={X.shape[1]} vs win_size={win_size}"
        )

    if input_c is not None and X.shape[2] != input_c:
        raise ValueError(
            f"Loaded feature dim mismatch: X.shape[2]={X.shape[2]} vs input_c={input_c}"
        )

    print(f"[CV-LOAD] dataset={dataset} source={source}")
    print(f"[CV-LOAD] X shape: {X.shape}")
    print(f"[CV-LOAD] y shape: {y.shape}")
...
def run_our_model(
    ...
    **kwargs,
    ...
    solver.thre_loader = test_loader
